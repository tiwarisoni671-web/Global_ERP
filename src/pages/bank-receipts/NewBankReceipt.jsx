...
const NewBankReceipt = () => {
  const [form, setForm] = useState({
    rctNo: 'BRCT-' + Date.now().toString().slice(-6),
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (form.amount <= 0) {
      alert("Please enter a valid bank receipt amount.");
      return;
    }
    // Simulate Bank Ledger Update
    const balMsg = `Success: Updated Bank Ledger! Credited ₹ ${form.amount.toLocaleString()} in account "${form.bankAccount}" from "${form.customer}" (Reference: ${form.refNo || 'N/A'}). Ledger reconciled.`;
    setLedgerMessage(balMsg);
    alert("Bank receipt voucher saved successfully and bank ledger updated!");
  };
  ...
  return (
    <div className="bg-white p-4 sm:p-6 rounded-lg border border-slate-200 shadow-sm min-h-screen space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 border-b pb-4">
        <div>
          <h1 className="text-lg sm:text-xl font-bold text-gray-800">New Bank Receipt Voucher</h1>
          <p className="text-[11px] sm:text-xs text-gray-500">Record customer bank transfers, process cheque clearings, ledger updates, and track reconciliation parameters.</p>
        </div>
        <button
          type="button"
          onClick={handlePrint}
      ...
      <form onSubmit={handleSubmit} className="space-y-4 text-xs sm:text-sm">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
          ...
          <div>
            <label className="block text-[10px] sm:text-xs font-semibold text-gray-700 uppercase mb-1">Clearance Date *</label>
            <input
              type="date"
              required
              value={form.date}
          ...
          <div>
            <label className="block text-[10px] sm:text-xs font-semibold text-gray-700 uppercase mb-1">Deposit Bank Account *</label>
            <select
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 border-t pt-4">
          <div>
            <label className="block text-[10px] sm:text-xs font-semibold text-gray-700 uppercase mb-1">Party / Customer Selection *</label>
            <select
              value={form.customer}
              onChange={(e) => setForm({ ...form, customer: e.target.value })}
              className="w-full border p-2 rounded focus:outline-none bg-white text-xs"
            >
              <option value="Ramesh Kumar & Sons">Ramesh Kumar & Sons</option>
              <option value="Apex Retailers">Apex Retailers</option>
            </select>
          </div>
          <div>
            <label className="block text-[10px] sm:text-xs font-semibold text-gray-700 uppercase mb-1">Amount Deposited (₹) *</label>
            <input
              type="number"
              required
              value={form.amount}
          ...
          <div>
            ...
            >
              ...
              <option value="Advance">Mark as Advance Deposit</option>
            </select>
          </div>
        </div>
        ...
        <div className="flex gap-2 justify-end pt-3 border-t no-print">
          <button type="submit" className="px-5 py-2 bg-blue-600 text-white rounded font-semibold hover:bg-blue-700 transition-colors text-xs">
            Save Bank Receipt & Update Ledger
          </button>
        </div>
      </form>
    </div>
